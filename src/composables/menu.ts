...
export function menu() {
  ...
  const getFoodGroupsAndFoodsByCompanyName = async (data: any) => {
    try {
      let response = await api.get(
        `/Menu/get-food-groups-and-foods-by-company-name/${data}`
      );
  ...
  const addFoodGroup = async (data: any) => {
    try {
      let response = await api.post("/Menu/add-food-group", data, {
        headers: {
  ...
  const getMenu = async (data: any) => {
    try {
      //data=9;
      let response = await api.get(`/Menu/get-menu/${data}`);
      console.log("response -> ", response);
  ...
  const getCompanyByUserId = async () => {
    try {
      let response = await api.get(`/Menu/get-company-by-user-id/`);
      console.log("response -> ", response);
